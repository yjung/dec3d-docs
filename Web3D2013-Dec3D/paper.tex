...
DFKI's XML3D \cite{Sons2010} is a similar approach. In contrast to X3DOM, this format proposes a new tag set that is not based on X3D anymore. Furthermore, it does not define a scene-graph but a strict tree structure by means of a prototyping mechanism. In recent versions of XML3D, mesh data can also be externalized from the HTML document by storing it in separate JSON files. Moreover, the developers are working on a system for dynamic data-flow to efficiently support animations \cite{KleinSJRSB12}.
...
% TODO: don't cite this link but present the whole idea here!!!
% http://www.w3.org/community/declarative3d/wiki/Level_of_Integration/
Recently, the group proposed to measure a level of integration for 3D graphics in terms of W3C technologies (DOM, CSS. etc.). The proposal aims to explain existing and possible integration levels (Figure \ref{fig:polyInt}) and finally to identify an integration level the group is aiming for.
% again, this is still bla bla
% instead, insert text from slides - see slide 22 ff.
...
Polyfill-based approaches allow deriving hard requirements for related and utilized W3C standards and user agent APIs immediately. This leads to much more evolving concepts and solutions in contrast to an overall declarative 3D specification.
...
TODO % see slides, where these are discussed starting with slide 11/12

Dec3D Scene Graph
Modern and minimalistic scene graph or tree
traversal and culling
creates mesh/material/transform bundles for render layer
picking
Supports graph and tree structures for data reuse
API layer, no DOM integration!

Essential Dec3D DOM Elements
Focus on minimalistic element set, not usability
Represents SG concepts as DOM elements
Adds DOM, Events and CSS to SG

Comprehensive Dec3D DOM Elements
Builds on Essential and/or SG layer
Focus on usability not minimalism
Could build on existing SG standards (e.g. X3D) for convenience
...
Needs to be supported in UAs!

3. Hierarchy
HTML hierarchy
Comes with issues (e.g. CSS inheritance)
...
Name: Type, TupleSize, Value
Based on TypedArrays with interface to modify data

6. External Data Containers
External (binary) blobs / transmission format
Maps to generic data containers

7. Operators on Data Containers
Allows dynamic meshes, image processing, compression
Data driven approach: Xflow, X3D Routes, SVG Filter FX

8. Shaders
2-3 powerful fixed-function shaders
Enough for many use-cases
No timing attack issues
Can adapt to target device

9. Textures
Use HTML elements: \verb|<img>|, \verb|<video>|, \verb|<canvas>|
Interactive textures: \verb|<svg>|, \verb|<html>|

10. 3D Geometry?
Close to OpenGL primitives

11. Lights
Shader parameters in GL, but don't obstruct future concepts
...
14. DOM Events
Support standard events: mouseover, click
3D-specific DOM Events contain 3D-specific context information

15. Interfaces and Types?
Provide IDLs for new elements
Convenience functions
Dec3D Base Types: Lean set of complex data types for 2D/3D
Provide / use types that help to manage 3D scene, e.g. CSS Matrix
...
During the 1\textsuperscript{st} International Workshop on Declarative 3D for the Web Architecture \cite{Dec3D2012} as well as during more informal group meetings at Web3D and SIGGRAPH in 2011/2012, the members and supporters of the Declarative 3D W3C Community Group agreed that what is needed to make the effort successful and for the W3C to adopt/develop a Dec3D standard include the following topics.
